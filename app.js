...
async function processBatchFiles(files) {
  ...
  if (detectedType === 'price') {
    ...
    toast(`File Harga (${allRows.length} baris).`);
  } else if (detectedType === 'meta') {
    elPillDate.textContent = "METADATA";
    elPillRows.textContent = allRows.length.toLocaleString() + " (Info)";
    toast(`File Sektor (${allRows.length} baris).`);
...
elFileInput.onchange = async (e) => {
  ...
  if (tempXlsxFiles.length > 0) toast(`${tempXlsxFiles.length} XLSX, ${csvFiles.length} CSV. Convert dulu.`);
...
elBtnUpload.onclick = async () => {
  try { const { rows, type } = lastParsed; if (!rows.length) return; 
    if (type === 'meta') { toast("Update Info Saham..."); const { error } = await sb.from("symbols").upsert(rows, { onConflict: "symbol" }); if (error) throw error; toast("Info Sektor Updated!"); await loadMetadata(); } 
    else { toast("Upload Harga..."); await upsertSymbols(rows); await upsertPrices(rows); toast("Harga Updated."); }
    elBtnUpload.disabled = true; if (type === 'price') await refreshSignals();
  } catch(err) { console.error(err); toast("Upload gagal", true); }
};
...
elBtnAnalyze.onclick = async () => {
  try {
    ...
    let symbols = []; const { data } = await sb.from("prices_daily").select("symbol").eq("trade_date", targetDate); symbols = data.map(s => s.symbol);
    ...
    toast(`Simpan ${out.length} sinyal...`); await upsertSignals(out); cachedSignals = []; toast("Selesai."); await refreshSignals();
...
function renderSignals(rows) {
  ...
  elSignals.querySelector("tbody").innerHTML = d.map(r => {
    ...
    <tr>
      ...
      <td class="mono font-bold">${escapeHtml(r.symbol)} ${shariaBadge}</td>
